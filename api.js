...
const UsersAPI = {
  ...
  toggleWishlist: (id)   => apiFetch(`/users/wishlist/${id}`, { method: 'POST' }),
...
function updateNavbarAuthState() {
  ...
  if (user) {
    loginBtn.textContent = user.name?.split(' ')[0] || 'Account';
    loginBtn.onclick = () => showToast(`Hello, ${user.name}! 👋`, 'fa-circle-user');
  } else {
